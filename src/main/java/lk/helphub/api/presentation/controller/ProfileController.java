...
public class ProfileController {
        ...
        @ApiResponses({
                        ...
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "401", description = "Unauthorized", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiResponse.class), examples = @ExampleObject(value = "{\"status\":false,\"status_code\":\"UNAUTHORIZED\",\"message\":\"Full authentication is required to access this resource\"}")))
        })
        ...
        @Operation(summary = "Update user profile", description = "Updates profile fields including full name, display name, bio, birthday, and handles file uploads for profile picture and identity verification.")
        ...
        public ResponseEntity<ApiResponse<ProfileResponse>> updateProfile(
                        ...
                        @RequestPart(value = "identityVerification", required = false) org.springframework.web.multipart.MultipartFile identityVerification,
                        @RequestPart(value = "educationCertificates", required = false) java.util.List<org.springframework.web.multipart.MultipartFile> educationCertificates) {
        ...
        @Operation(summary = "Set language preference", description = "Sets the app-wide language preference (SINHALA, ENGLISH, or TAMIL). Triggers localized content across the app.")
        @ApiResponses({
        ...
        @DeleteMapping
        @Operation(summary = "Delete user profile", description = "Permanently (soft) deletes the user profile. Requires a valid OTP sent to the user's registered phone number. The delete reason is saved for compliance.")
        @ApiResponses({
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "Profile deleted successfully"),
                        @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "Invalid or expired OTP", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ApiResponse.class), examples = @ExampleObject(value = "{\"status\":false,\"status_code\":\"BAD_REQUEST\",\"message\":\"Invalid or expired OTP\"}"))),
